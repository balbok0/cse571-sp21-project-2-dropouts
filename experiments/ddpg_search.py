...
def evaluate_model(args, config):
    ...
    if mode == "rgb_array":
        save_video(images, video_path,  round(len(images) / sim_env.timestamp))
...
if __name__ == '__main__':
    ...
    if args.eval:
        ...
        register_env(
            "DuckieTown-MultiMap",
            lambda _: CustomRewardWrapper(MultiMapSteeringToWheelVelWrapper(MultiMapEnv(max_steps=75)))
        )
